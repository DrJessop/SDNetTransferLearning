...
def get_tensor_from_1p5t_data():
    ...
    for im in os.listdir(conf.data_1p5t):
        ...
        for idx in range(image.shape[0]):
            normalize(image[idx])
        sitk_image_normalized = sitk.GetImageFromArray(image.cpu().numpy())
        sitk_image_normalized.CopyInformation(sitk_image)
        path = os.path.join(conf.data_dest, im)
        sitk.WriteImage(sitk_image_normalized, path)
